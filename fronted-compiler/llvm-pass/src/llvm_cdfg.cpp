	...
	for(LLVMCDFGNode* node : phiNodes){
        ...
        if((!_loopBBs.count(PHI->getParent())) && (PHItoLevel.count(PHI))){
            ...
            for(auto itr = outputs.begin(); itr != outputs.end(); itr++){
                ...
                temnode->setInputIdx(prenode, inputidx);
		...
		for (int i = 0; i < PHI->getNumIncomingValues(); ++i) {
            ...
            if(ConstantInt* CI = dyn_cast<ConstantInt>(V)){
                ...
                if(Instruction* ins = dyn_cast<Instruction>(V)){
                    ...
                    if(dyn_cast<PHINode>(ins)){
                        if(phi2selectMap.count(phiParent)){ // PHI parent has been transformed to SELECT node
...
void LLVMCDFG::affineAnalyze(){
    ...
    for(auto iter = PHItoLevel.begin(); iter != PHItoLevel.end(); iter++){
        if(iter->second == 0)
            continue;
        auto ins = iter->first;
        LLVMCDFGNode* phinode = node(ins);
        ins->dump();
        for(User *succ : ins->users()){               
            if(Instruction *succIns = dyn_cast<Instruction>(succ)){
                if(auto succNode = node(succIns)){
                    errs()<<"\t";succIns->dump();
                    int idx = -1;
                    // reorder the operand of SELECT and STORE node
                    if (auto SI = dyn_cast<SelectInst>(succIns))
                    ...
                    EdgeType type = EDGE_TYPE_DATA;
                    if (ins->getType()->getPrimitiveSizeInBits() == 1)
                    { //  single-bit operand
                        type = EDGE_TYPE_CTRL;
                    }
                    // node->addOutputNode(succNode, isBackEdge);
                    // succNode->addInputNode(node, idx, isBackEdge);
                    // addEdge(node, succNode, type);
                    phinode->addOutputNode(succNode, false);
                    succNode->addInputNode(phinode, idx, false);
                    addEdge(phinode, succNode, type);
                }
            }
        } 
    }
    ...
    for(auto &elem : _nodes){
        ...
        if(node->hasConst()){
            ...
            if(ins == NULL){//innermost PHI
                ...
                errs() << "ACC " << name << " have IR instruction;\n";
                //ins->dump();
                ...
                int levels = getLoopsAffineCounts().size();
                errs() << "getLoopsAffineCounts().size() = "<<levels<<"\n";
                for(int i = temlevel+1; i < levels; i++){
