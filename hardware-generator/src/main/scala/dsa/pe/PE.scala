...
//  apply(attrs)
  ...
  val accOperandNum = ops.map(e => if(OpInfo.isAccumulative(e) > 0) {OpInfo.getOperandNum(e)} else {0}).max
  ...
  val imuxs = numInPerOperand.map{ num => Module(new Muxn(width, num+excdnnuxIns)).io } // input + const + (rf_out)
...
class SPE(accType: SInt, attrs: mutable.Map[String, Any]) extends GPE(attrs){
  val alu = Module(new ALU(accType, width, ops))
  ...
  val currentPorts = RegInit(0.U(2.W))
  if(hasAcc){
    dmr.io.start := io.start
    isAccOp := OpInfo.isAccOp(alu.io.config)
    currentPorts := OpInfo.getCurrentOperandNum(alu.io.config)
  ...
  if(hasAcc && aluOperandNum == accOperandNum){
    alu.io.in(accOperandNum) := dmr.io.out(1)
    connections.append((smi_id("REG")(0), "REG", 1, smi_id("ALU")(0), "ALU", accOperandNum))
  }
  ...
  if(delayCfgWidth != 0){
    ...
    configuration += smi_id("RDU")(0) -> ("RDU", offset+delayCfgWidth-1, offset)
  } else{
  ...
  if(hasAcc){
    when(isAccOp){
      when(currentPorts === 1.U){
        if(aluOperandNum > 1)
          imuxs(1).config := (imuxs(1).in.size - 1).asUInt
      }
      when(currentPorts === 2.U){
        if(aluOperandNum > 2)
          imuxs(2).config := (imuxs(2).in.size - 1).asUInt
      }
    }
  }
...
class FPE(accType: Float, attrs: mutable.Map[String, Any])extends GPE(attrs) {
  val fpu = Module(new FPU(accType, width, ops))
  val const = Wire(UInt(width.W))
  ...
  delay_pipe.io.en := io.en
//  dmr.io.en := io.en
  dmr.io.in := fpu.io.out
  io.out(0) := dmr.io.out(0)
  val isAccOp = RegInit(false.B)
  val currentPorts = RegInit(0.U(2.W))
  if(hasAcc){
    dmr.io.start := io.start
    isAccOp := OpInfo.isAccOp(fpu.io.config)
    currentPorts := OpInfo.getCurrentOperandNum(fpu.io.config).asUInt
    dmr.io.bypass := !isAccOp
  }else{
    dmr.io.start := DontCare
    dmr.io.bypass := DontCare
  ...
  for(i <- 0 until aluOperandNum){
    ...
    delay_pipe.io.in(i) := imuxs(i).out
    fpu.io.in(i) := delay_pipe.io.out(i)
    connections.append((smi_id("Muxn")(i), "Muxn", 0, smi_id("RDU")(0), "RDU", i))
    connections.append((smi_id("RDU")(0), "RDU", i, smi_id("ALU")(0), "ALU", i))
    offset += num
  }
  if(hasAcc && aluOperandNum == accOperandNum){
    fpu.io.in(accOperandNum) := dmr.io.out(1)
    connections.append((smi_id("REG")(0), "REG", 1, smi_id("ALU")(0), "ALU", accOperandNum))
  }
  ...
  offset = constCfgWidth
  if(aluCfgWidth != 0){
    fpu.io.config := cfgOut(offset+aluCfgWidth-1, offset)
    configuration += smi_id("ALU")(0) -> ("ALU", offset+aluCfgWidth-1, offset)
  } else {
    fpu.io.config := DontCare
  }
  offset += aluCfgWidth
  if(delayCfgWidth != 0){
    delay_pipe.io.config := cfgOut(offset+delayCfgWidth-1, offset)
