...
//    connect
  ...
  def getOPin2TrackConnect(numTrack: Int, fc: Int, numPinList: List[Int], trackDirections : ListBuffer[Int]): ListBuffer[Seq[Int]] = {
    ...
    val w = numTrack
    val fcStep : Double = 2*w.toDouble/fc
...
//    connect
   ...
   *  connect an opin in one corner to the ipins on the other one or two or three corners, default: the left corner
   * 
   * @param fc      connection flexibility of the output pins, 1n or 2n
   * @param diag    if support diagonal connections
   * @param right   if support right corner connections
   * @param numIOPinList  number of the PE I/O Pins on four sides
   */
  def getOPin2IPinConnect(fc: Int, diag: Boolean, right: Boolean, numIOPinList: List[Int]): ListBuffer[Seq[Int]] = {
    // numIOPinList
    ...
    val nSides = {
      if(diag && right) 3
      else if(diag || right) 2
      else 1
    }
    val fcPerSide = fc/nSides // { if(diag) fc/2 else fc }
    val connect = ListBuffer[Seq[Int]]() // (srcSide, srcTrack, dstSide, dstTrack)
    val fcNWstep = nNWi/fcPerSide.toDouble
    val fcNEstep = nNEi/fcPerSide.toDouble
    val fcSEstep = nSEi/fcPerSide.toDouble
    val fcSWstep = nSWi/fcPerSide.toDouble
    for(i <- 0 until nNWo){
   ...
   * @param rightPinConect if support OPin connected to IPins in right corner
   * @param numIOPinList  number of the PE I/O Pins on four sides
   * @param fcList        connect flexibility
   * @param trackDirections  which directions have valid track IO ports
   */
  def getGIBConnect(numTrack: Int, diagPinConect: Boolean, rightPinConect: Boolean, numIOPinList: List[Int],
                    fcList: List[Int], trackDirections : ListBuffer[Int]): ListBuffer[(Seq[Int], ListBuffer[Seq[Int]])] = {
    var dstList = ListBuffer[Seq[Int]]()
    var srcsList = ListBuffer[ListBuffer[Seq[Int]]]() // srcs connected to each dst
    val oPin2IPinConnect = getOPin2IPinConnect(fcList(2), diagPinConect, rightPinConect, numIOPinList)
...
class GIB(attrs: mutable.Map[String, Any]) extends Module with IR {
  ...
  // if support OPin connected to IPins in right corner
  val rightPinConect = false // attrs("right_iopin_connect").asInstanceOf[Boolean]
  // numIOPinList
...
//  val fcList = List(fci, fco, fcio)
  ...
  // wire connections
  val connect = GIB.getGIBConnect(numTrack, diagPinConect, rightPinConect, numIOPinList, fcList, trackDirections)
  ...
  if(cfgsBit > 0) {
    ...
    connect.map { case (dst, srcs) =>
      if(srcs.size > 1) {
        ...
        if (trackReged && (dst(0) == TYPE_OTRACK)) {
          wireMap((dst(0), dst(1)))(dst(2)) := RegNext(wireMap((src(0), src(1)))(src(2)))
        } else {
          wireMap((dst(0), dst(1)))(dst(2)) := wireMap((src(0), src(1)))(src(2))
        }
      }
