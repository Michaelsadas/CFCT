 ...
 */ 
class CGRA(sint_type: SInt, float_type: Float, attrs: mutable.Map[String, Any]) extends Module with IR{
  // CGRA parameters
  ...
  val iob_attrs: mutable.Map[String, Any] = mutable.Map(
    ...
    "lg_max_preload" -> lgMaxPreload,
    "lg_max_wait" -> lgWait,
    "lg_max_store" -> lgStore,
    "max_delay" -> 4,
    "num_input_per_operand" -> ListBuffer()
  )
...
//      if(j == 4) pes += Module(new SPE(sint_type, gpe_attrs))
//      else pes += Module(new FPE(float_type, gpe_attrs)) 
      //pes += Module(new SPE(sint_type, gpe_attrs)) 
      pes += Module(new FPE(float_type, gpe_attrs))
