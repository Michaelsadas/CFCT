...
case class IobParam(
  ...
  val num_input_per_operand = {
    val numOperand = {
      if(mode == SRAM_MODE) input_operand // address and data
      else 1  // data
    }
...
//    SERIAL_MODE -> "SERIAL_MODE"
  ...
  val iobsParam = iobsSpec.map{ buf =>
    buf.map{ spec => IobParam(spec.mode, spec.max_delay, spec.input_operand) }
  }
  ...
  val lgMaxPreload = attrs("lg_max_preload").asInstanceOf[Int]
  val lgWait = attrs("lg_max_wait").asInstanceOf[Int]         
  val lgStore = attrs("lg_max_store").asInstanceOf[Int]
  val iob_mode = attrs("cgra_iob_mode")
