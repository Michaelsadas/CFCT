...
//  apply(attrs)
  ...
  val lgMaxPreload = attrs("lg_max_preload").asInstanceOf[Int]    // log2(max out preload time in one loop)
  val lgWait = attrs("lg_max_wait").asInstanceOf[Int]             // log2(max out wait time in one loop)
  val lgStore = attrs("lg_max_store").asInstanceOf[Int]           // log2(max store time in one loop)
  // cfgParams
  ...
  val numOut = 1
  val numInCtrl = { if(mode == FIFO_MODE) 1 
                    else numInList.size}
  ...
  if(mode != FIFO_MODE){ assert( numInCtrl >= 2 ) }
...
//    val cycles = Input(UInt(lgMaxCycles.W)) // valid in/out cycles, shared among all IOB, so not put in private config
  ...
  val ioCtrl = Module(new IOController(width, addrWidthSram, hasMaskSram, mode, numInCtrl, lgMaxII, lgMaxLat, lgMaxStride, lgMaxCycles, agNestLevels, lgMaxPreload, lgWait, lgStore, addRegSram))
...
//  val delay_pipes = Array.fill(numDelayPipes){ Module(new DelayPipe(width, maxDelay)).io }
  ...
  if(numDelayPipes > 0) {
    sm_id += ("RDU" -> 3)
//    smi_id += ("RDU" -> (numInCtrl+2 until numDelayPipes+numInCtrl+2).toList)
  ...
  if(delayCfgWidth != 0){
    ...
    configuration += smi_id("RDU")(0) -> ("RDU", delayCfgWidth+offset-1, offset)
