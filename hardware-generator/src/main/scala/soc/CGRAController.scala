...
/** CGRA controller
  ...
  */
class CGRAController(sint_type: SInt, float_type: Float, attrs: mutable.Map[String, Any])(implicit p: Parameters) extends CoreModule {
...
//  val lgMaxII = attrs("cgra_exe_lg_max_ii").asInstanceOf[Int]             // log2(max in/out Initialization Interval)
  ...
  val cgra = Module(new CGRA(sint_type, float_type, attrs))
...
//  val exeReqState = RegInit(s_req_idle)
  val s_idle :: s_cfg_run :: s_cfg_resp :: s_exe_run :: s_exe_resp :: Nil = Enum(5)
...
//  val cfg_rs1 = cmd.rs1.asTypeOf(new CfgRs1)
  ...
  io.core.req.ready := ((state === s_idle) && isCfgCmd) || ((state === s_idle) && isExeCmd)
  switch(state){
    is(s_idle){
      when(io.core.req.valid && isCfgCmd){
        state := s_cfg_run
