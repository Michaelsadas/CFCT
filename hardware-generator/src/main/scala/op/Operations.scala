...
//---------------------------------------------------------------------------------------------------------------------------------//
		...
		"FADD" -> ListBuffer(2, 1, 1, 1, 0),
		"FSUB" -> ListBuffer(2, 1, 1, 0, 0),
		"FMUL" -> ListBuffer(2, 1, 1, 1, 0),
		"FDIV" -> ListBuffer(2, 1, 1, 0, 0), //The latency of divide is based on the length of the float
		"FMOD" -> ListBuffer(2, 1, 1, 0, 0),
		"FMIN" -> ListBuffer(2, 1, 1, 1, 0),
		"FMAX" -> ListBuffer(2, 1, 1, 1, 0),
		"FSHL" -> ListBuffer(2, 1, 1, 0, 0),
...
//--------------------------------------------------------------------------------------------------------------------------------//
		"FACC"  -> ListBuffer(1, 1, 1, 0, 1),  // +=
		"FASUB" -> ListBuffer(1, 1, 1, 0, 1), // -=
		"FAMUL" -> ListBuffer(1, 1, 1, 0, 1), // *=
		"FADIV" -> ListBuffer(1, 1, 1, 0, 1), // /=
		"FAMOD" -> ListBuffer(1, 1, 1, 0, 1), // %=
		"FASHL" -> ListBuffer(1, 1, 1, 0, 1), // <<=
...
//-------------------------------------------------------------------------------------------------------------------------------//
	...
	def Float_OpFuncs(accType: Float, ops: Seq[UInt], opc: UInt, opset: ListBuffer[OPC])(implicit ev: Arithmetic[Float]): Map[String, Seq[UInt]] = {
		import ev._
		...
		Map(
			"FPASS" -> Seq(op0),
			"FADD"  -> Seq((opa + opb).asUInt),
			"FSUB"  -> Seq((opa - opb).asUInt),
			"FMUL"  -> Seq((opa * opb).asUInt),
			"FUDIV" -> Seq((opa / opb).asUInt),
			"FMOD"  -> Seq((opa % opb).asUInt),
			"FMIN"  -> Seq(Mux(opa < opb, opa.asUInt, opb.asUInt)),
			"FMAX"  -> Seq(Mux(opa > opb, opa.asUInt, opb.asUInt)),
			"FSHL"  -> Seq((opa << shn).asUInt),
			...
			"FACC"   -> Seq((opb + opa).asUInt), // the SECOND operand is the register value. the following is similar
