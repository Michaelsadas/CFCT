...
void cgra_execute(void** din_addr, void** dout_addr)
{
	...
	load_cfg((void*)cin, 0x20000, 684, 1);
	load_data(din_addr[0], 0x18000, 6400, 0, 1);
	load_data(din_addr[1], 0x12000, 6400, 0, 1);
	load_data(din_addr[2], 0x0, 6400, 0, 1);
  	load_data(din_addr[3], 0x10000, 36, 0, 1);
	load_data(din_addr[4], 0x14000, 36, 0, 1);
	load_data(din_addr[5], 0x2000, 36, 0, 1);
	load_data(din_addr[6], 0x1a000, 36, 0, 1);
	load_data(din_addr[7], 0x1c000, 36, 0, 1);
	load_data(din_addr[8], 0x4000, 36, 0, 1);
	load_data(din_addr[9], 0x6000, 36, 0, 1);
	load_data(din_addr[10], 0x8000, 36, 0, 1);
	load_data(din_addr[11], 0xa000, 36, 0, 1);
//    load_data(din_addr[3], 0x10000, 40, 0, 1);
//	load_data(din_addr[4], 0x14000, 40, 0, 1);
//	load_data(din_addr[5], 0x2000, 40, 0, 1);
//	load_data(din_addr[6], 0x1a000, 40, 0, 1);
//	load_data(din_addr[7], 0x1c000, 40, 0, 1);
//	load_data(din_addr[8], 0x4000, 40, 0, 1);
//	load_data(din_addr[9], 0x6000, 40, 0, 1);
//	load_data(din_addr[10], 0x8000, 40, 0, 1);
//	load_data(din_addr[11], 0xa000, 40, 0, 1);
	config(0x0, 114, 1);
	execute(0xff7f, 1);
	store(dout_addr[0], 0x16000, 5776, 1);
	store(dout_addr[1], 0x1e000, 5776, 1);
	store(dout_addr[2], 0xc000, 5776, 1);
}
...
void check(){
    for (int oc = 0; oc < OC; oc ++){
        for (int h = 0; h < OH; h = h + SH * 10) {
            for (int w = 0; w < OW; w = w + SW * 10) {
                if(R[oc][h][w] != B[oc][h][w]) printf("Wrong in location (%d, %d): True: %d, Wrong: %d\n", h, w, R[oc][h][w], B[oc][h][w]);
            }
        }
    }
}
