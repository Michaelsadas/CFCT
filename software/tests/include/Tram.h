...
static inline int load_cfg(void *dram_ptr, int spad_ptr, int len, int target_cdnn)
{
//	uint64_t value;
...
static inline int load_data(void *dram_ptr, int spad_ptr, int len, int fused, int target_cdnn)
{
//	uint64_t value;
...
static inline int reload_data(int spad_ptr, int target_cdnn)
{
...
static inline int load_set(int loop, int spad_stride, int remote_stride, int target_cdnn)
{
...
static inline int store(void *dram_ptr, int spad_ptr, int len, int target_cdnn)
{
//	uint64_t value;
...
static inline int store_set(int loop, int spad_stride, int remote_stride, int target_cdnn)
{
	uint64_t rs2 = ((uint64_t) loop << 44) | ((uint64_t) spad_stride << 22) | (uint64_t) remote_stride;
	if (target_cdnn == 0)
...
static inline int config(int cfg_base_addr, int cfg_num, int target_cdnn)
{
	uint64_t rs1 = 0;
...
static inline int execute(uint64_t iob_ens, int target_cdnn)
{
...
static inline int fence(int xd, int target_cdnn)
{
...
static inline int flush(unsigned target_cdnn)
{
	unsigned long value;
	if (target_cdnn == 0)
