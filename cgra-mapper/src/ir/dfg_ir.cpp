...
DFG* DFGIR::parseDFGJson(std::string filename){
    ...
    for(auto& nodeJson : dfgJson["objects"]){
        ...
        if(opName == "CONST"){
            ...
            DFGNode* dfg_node;            
            if(opName == "INPUT" || opName == "OUTPUT" || opName == "LOAD" || opName == "STORE" || opName == "SSTORE" || opName == "CLOAD" || opName == "CSTORE"){
                dfg->addIONode(id);
                ...
                if(nodeJson.contains("serial")){
                    std::string serialStr = nodeJson["serial"].get<std::string>();
                    std::vector<uint64_t> values = strSplit2Uint64(serialStr, ',');
                    //assert(values.size() >= 4);
                    dfg_io_node->setPreloadTime(values[0]);
                    dfg_io_node->setWaitTime(values[1]);
                    dfg_io_node->setStoreTime(values[2]);
                    //dfg_io_node->setUseSerial(values[3]);
                    dfg_io_node->setUseAccum(values[3]);
                }
