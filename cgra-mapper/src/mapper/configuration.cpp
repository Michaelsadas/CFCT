...
std::map<int, CfgData> Configuration::getIobCfgData(IOBNode* node){
    ...
    int useAccum = dfgIONode->useAccum();
    //int useSerial = dfgIONode->useSerial();
    int waitTime = dfgIONode->waitTime();
    int preloadTime = dfgIONode->preTime();
    int storeTime = dfgIONode->storeTime();
    ...
    if(node->cfgIdMap.count("UseAccum")){
        int useAccumId = node->cfgIdMap["UseAccum"];
        addCfgData(cfg, node->configInfo(useAccumId), (uint32_t)useAccum);
    }
    if(node->cfgIdMap.count("Waittime")){
        int WaitId = node->cfgIdMap["Waittime"];
        addCfgData(cfg, node->configInfo(WaitId), (uint32_t)waitTime);
    }
    if(node->cfgIdMap.count("Preloadtime")){
        int PreId = node->cfgIdMap["Preloadtime"];
        addCfgData(cfg, node->configInfo(PreId), (uint32_t)preloadTime);
    }
    if(node->cfgIdMap.count("Storetime")){
        int StoreId = node->cfgIdMap["Storetime"];
        addCfgData(cfg, node->configInfo(StoreId), (uint32_t)storeTime);
    }
    if(node->cfgIdMap.count("UseSerial")){
        int useSerial = op == "SSTORE";
        int useSerialId = node->cfgIdMap["UseSerial"];
        addCfgData(cfg, node->configInfo(useSerialId), (uint32_t)useSerial);
    }
